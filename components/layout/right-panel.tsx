...
export function RightPanel() {
  ...
  useEffect(() => {
    const getChainIdFromWallet = async () => {
      if (
        ...
        try {
          const hexId = await (window as any).ethereum.request({
            method: "eth_chainId",
          });
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      if (isConnected && address) {
        ...
        try {
          console.log(
            "Fetching right panel analytics for wallet:",
            address,
            "on chain:",
            actualChainId
          );
          const res = await fetch(
            `/api/analytics?address=${address}&chainId=${actualChainId}`
          );
          if (res.ok) {
            const json = await res.json();
            setAnalytics(json.data);
            console.log("Right panel analytics data received:", json.data);
          } else {
  ...
  const totalVolume =
    analytics?.recentTransactions?.length > 0
      ? analytics.recentTransactions.reduce(
          ...
          <div className="space-y-4">
              ...
              <div className="text-right">
                <div className="text-white font-semibold">
                  {loading
                    ...
                    : analytics?.recentTransactions?.length > 0
                    ? `${(
                        totalVolume / analytics.recentTransactions.length
                      ).toFixed(6)} ETH`
                    : "0.000000 ETH"}
                </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/10">
              ...
              <div className="text-right">
                ...
                <div className="text-gray-400 text-xs">Real count</div>
              </div>
            </div>
          </div>
