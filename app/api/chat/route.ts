...
import {
  parseUserIntent,
  executeHederaOperation,
  getHederaBalance,
} from "@/lib/hederaAgent";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isBlockscoutQuery(message)) {
      try {
        const blockscoutResponse = await handleBlockscoutQuery(
          message,
          walletAddress,
          chainId
        );
        ...
        return NextResponse.json({
          response: `❌ Failed to process Blockscout query: ${
    ...
    switch (agentResponse.intent.action) {
      ...
      case "bridge":
        if (agentResponse.intent.chainFrom && agentResponse.intent.chainTo) {
          try {
            ...
            status =
              bridgeResult.status === "submitted"
                ? "pending"
                : bridgeResult.status;

            const txDisplay = bridgeResult.transactionHash
              ? `${bridgeResult.transactionHash.slice(0, 10)}...`
              : bridgeResult.bridgeId
              ? `ID: ${bridgeResult.bridgeId}`
              : "n/a";
            ...
            if (status === "success") {
              response =
                ...
                `**Route:** ${getChainName(
                  agentResponse.intent.chainFrom
                )} → ${getChainName(agentResponse.intent.chainTo)}\n` +
                `**Gas Cost:** ${bridgeResult.gasCost}\n` +
                `**Estimated Time:** ${bridgeResult.estimatedTime}\n` +
                `**Transaction:** ${txDisplay}\n\n` +
        ...
        if (walletAddress) {
          try {
            ...
            const analyticsResponse =
              await serverBlockscoutService.getWalletAnalytics(
                walletAddress,
                chainId || 1
              );
            ...
            if (analyticsResponse.success && analyticsResponse.data) {
              if (Array.isArray(analyticsResponse.data)) {
                response = serverBlockscoutService.formatTransactionHistory(
                  analyticsResponse.data
                );
              } else {
                response = serverBlockscoutService.formatWalletAnalytics(
                  analyticsResponse.data
                );
              }
              metadata = {
            ...
            response = `❌ Failed to fetch wallet analytics: ${
              error instanceof Error ? error.message : "Unknown error"
            }`;
            status = "failed";
          }
      ...
      case "hedera_operation":
        try {
          ...
          status = hederaResponse.executionResult?.success
            ? "success"
            : "failed";
          ...
          response = `❌ Failed to execute Hedera operation: ${
            error instanceof Error ? error.message : "Unknown error"
          }`;
          status = "failed";
        }
...
async function handleBlockscoutQuery(
  message: string,
  ...
  if (
    lowerMessage.includes("wallet-analytics") ||
    ...
    if (!address) {
      return {
        ...
        suggestions: [
          "Provide an address like: wallet-analytics 0x1234...5678",
        ],
      };
    }

    const result = await serverBlockscoutService.getWalletAnalytics(
      address,
      targetChainId as any
    );
    if (result.success && result.data) {
      if (Array.isArray(result.data)) {
        return {
          success: true,
          message: serverBlockscoutService.formatTransactionHistory(
            result.data
          ),
          data: result.data,
  ...
  if (
    ...
    if (!address) {
      return {
        ...
        suggestions: [
          "Provide an address like: transaction-history 0x1234...5678",
        ],
      };
    }
    ...
    const result = await serverBlockscoutService.getTransactionHistory(
      address,
      targetChainId as any,
      limit
    );
