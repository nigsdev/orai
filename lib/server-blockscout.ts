...
class ServerBlockscoutService {
   ...
   */
  async getWalletAnalytics(address: string, chainId: SupportedChainId): Promise<ServerBlockscoutResponse> {
    try {
      // Validate address
      ...
      const transactions = await this.fetchTransactionsFromAPI(chain.explorer, address)
      ...
      return {
        success: true,
        data: analytics,
        message: `📊 Wallet Analytics for ${this.formatAddress(address)} on ${chain.name}`,
        suggestions: [
   ...
   */
  async getTransactionHistory(address: string, chainId: SupportedChainId, limit: number = 10): Promise<ServerBlockscoutResponse> {
    try {
      if (!this.isValidAddress(address)) {
        return {
          success: false,
      ...
      const transactions = await this.fetchTransactionsFromAPI(chain.explorer, address, limit)
      
      return {
        success: true,
        data: transactions,
        message: `📋 Recent ${transactions.length} transactions for ${this.formatAddress(address)}`,
        suggestions: [
...
💡 Use the chain ID when requesting wallet analytics or transaction data.
  ...
  private async fetchTransactionsFromAPI(explorerUrl: string, address: string, limit: number = 50): Promise<TransactionData[]> {
    try {
      ...
      throw new Error(`Failed to fetch real transaction data from Blockscout: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
...
${transactions.slice(0, 5).map((tx, index) => {
  const directionText = tx.direction === 'sent' ? 'Sent to' : tx.direction === 'received' ? 'Received from' : 'Transaction'
  const counterparty = tx.counterpartyAddress ? `${tx.counterpartyAddress.slice(0, 6)}...${tx.counterpartyAddress.slice(-4)}` : 'Unknown'
  const statusIcon = tx.status === 'success' ? '✅' : tx.status === 'failed' ? '❌' : '⏳'
  const statusText = tx.status === 'success' ? 'Success' : tx.status === 'failed' ? 'Failed' : 'Pending'
  
  return `
${index + 1}. **${statusIcon} ${tx.hash.slice(0, 10)}...** (${statusText})
   📅 **Date:** ${tx.timestamp ? new Date(tx.timestamp).toLocaleDateString() : 'Pending'}
   💰 **Amount:** ${this.formatWei(BigInt(tx.value))} ETH
   📤 **${directionText}:** ${counterparty}
   ⛽ **Gas Fee:** ${tx.gasFeeEth} ETH
