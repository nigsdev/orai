...
export class ChatbotBlockscoutIntegration {
   ...
   */
  static async getWalletAnalytics(
    address: string,
    chainId: SupportedChainId
  ): Promise<ChatbotWalletResponse> {
    try {
      // Validate address
      ...
      const analytics = await blockscoutService.getWalletAnalytics(
        address,
        chainId
      );
      ...
      return {
        success: true,
        data: analytics,
        message: `📊 Wallet Analytics for ${blockscoutService.formatAddress(
          address
        )} on ${supportedChains[chainId].name}`,
        suggestions: [
  ...
  static async getTransactionHistory(
    ...
    try {
      ...
      const transactions = await blockscoutService.getTransactionHistory(
        address,
        chainId,
        limit
      );
      ...
      return {
        ...
        message: `📋 Recent ${
          transactions.length
        } transactions for ${blockscoutService.formatAddress(address)}`,
        suggestions: [
  ...
  static async monitorTransaction(
    chainId: SupportedChainId,
    txHash: string
  ): Promise<ChatbotTransactionResponse> {
    try {
      ...
      const explorerUrl = blockscoutService.getExplorerUrl(
        chainId,
        "tx",
        txHash
      );
   ...
   */
  static showTransactionHistoryPopup(
    chainId: SupportedChainId,
    address?: string
    ...
    try {
      ...
      return {
        success: true,
        message: address
          ? `📋 Showing transaction history for ${blockscoutService.formatAddress(
              address
            )}`
...
${transactions
  ...
  .map(
    (tx, index) => `
${index + 1}. **${tx.status === "success" ? "✅" : "❌"} ${tx.hash.slice(
      0,
      10
    )}...**
...
💡 Use the chain ID when requesting wallet analytics or transaction data.
   ...
   */
  static parseWalletInput(input: string): {
    address?: string;
    chainId?: SupportedChainId;
    error?: string;
  } {
    ...
    const chainId = chainIdMatch
      ? (parseInt(chainIdMatch[1]) as SupportedChainId)
      : undefined;

    if (!address) {
      return {
        error:
          "No valid wallet address found. Please provide an address starting with 0x",
      };
    }
   ...
   */
  static parseTransactionInput(input: string): {
    txHash?: string;
    chainId?: SupportedChainId;
    error?: string;
  } {
    ...
    if (!txHash) {
      return {
        error:
          "No valid transaction hash found. Please provide a hash starting with 0x",
      };
    }
