...
export class ChatbotBlockscoutCommandHandler {
   ...
   */
  private static async handleWalletAnalytics(args: string[]): Promise<ChatbotResponse> {
    if (args.length === 0) {
      return {
        success: false,
    ...
    const result = await chatbotBlockscout.getWalletAnalytics(address, chainId as SupportedChainId)
    
    if (result.success && result.data) {
      return {
        success: true,
        message: chatbotBlockscout.formatWalletAnalytics(result.data),
        data: result.data,
   ...
   */
  private static async handleTransactionHistory(args: string[]): Promise<ChatbotResponse> {
    if (args.length === 0) {
      return {
        success: false,
        message: 'Please provide a wallet address',
        suggestions: ['Usage: transaction-history <address> [chain-id] [limit]']
      }
    }
    ...
    const result = await chatbotBlockscout.getTransactionHistory(address, chainId as SupportedChainId, limit)
    
    if (result.success && result.data) {
      return {
        success: true,
        message: chatbotBlockscout.formatTransactionHistory(result.data as any[]),
        data: result.data,
        suggestions: result.suggestions
      }
    } else {
   ...
   */
  private static async handleMonitorTransaction(args: string[]): Promise<ChatbotResponse> {
    if (args.length === 0) {
      return {
        success: false,
    ...
    const result = await chatbotBlockscout.monitorTransaction(chainId as SupportedChainId, txHash)
   ...
   */
  private static async handleShowTransactionHistory(args: string[]): Promise<ChatbotResponse> {
    const address = args.find(arg => arg.startsWith('0x'))
    const chainId = this.parseChainId(args) || 1

    const result = chatbotBlockscout.showTransactionHistoryPopup(chainId as SupportedChainId, address)
   ...
   */
  private static async handleParseWallet(input: string): Promise<ChatbotResponse> {
    if (!input) {
      return {
        success: false,
    ...
    return {
      ...
      message: result.error || `Parsed: Address ${result.address}, Chain ${result.chainId}`,
      data: result,
      error: result.error
    }
  }
   ...
   */
  private static async handleParseTransaction(input: string): Promise<ChatbotResponse> {
    if (!input) {
      return {
        success: false,
        message: 'Please provide text to parse',
    ...
    return {
      success: !result.error,
      message: result.error || `Parsed: Transaction ${result.txHash}, Chain ${result.chainId}`,
      data: result,
      error: result.error
    }
  }
  ...
  private static async handleHelp(): Promise<ChatbotResponse> {
    const commandsList = BLOCKSCOUT_COMMANDS.map(cmd => 
  ...
  static isBlockscoutCommand(input: string): boolean {
    ...
    return BLOCKSCOUT_COMMANDS.some(cmd => cmd.command === command) || command === 'help'
  }
