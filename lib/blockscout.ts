...
class BlockscoutServiceImpl implements BlockscoutService {
   ...
   */
  async showTransactionToast(
    chainId: SupportedChainId,
    txHash: string
  ): Promise<void> {
    if (!this.notificationHook) {
      throw new Error(
        "Notification hook not available. Make sure to wrap your app with NotificationProvider."
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to show transaction toast: ${
          error instanceof Error ? error.message : "Unknown error"
        }`
      );
    }
  ...
  showTransactionHistory(chainId: SupportedChainId, address?: string): void {
    if (!this.popupHook) {
      throw new Error(
        "Transaction popup hook not available. Make sure to wrap your app with TransactionPopupProvider."
      );
    }
...
// Types are already exported above via 'export interface' and 'export type'

// Compatibility wrapper functions expected by app modules
export async function getRecentTransactions(
  address: string,
  chainId: number,
  limit: number = 10
): Promise<TransactionData[]> {
  return blockscoutService.getTransactionHistory(
    address,
...
export async function getTokenBalances(
  address: string,
  chainId: number
): Promise<Array<{ symbol: string; balance: string; value?: string }>> {
  // No direct token balances API here; return empty list as fallback
  return [];
}
...
export async function getWalletAnalytics(
  ...
  const txs = await blockscoutService.getTransactionHistory(
    address,
    chainId as SupportedChainId,
    50
  );
  ...
  txs.forEach((tx) => {
    tx.tokenTransfers?.forEach((transfer) => {
      const current = tokenBalanceMap.get(transfer.symbol) || 0;
      const amountNum = Number(transfer.amount || 0);
      tokenBalanceMap.set(
        transfer.symbol,
        current + (isFinite(amountNum) ? amountNum : 0)
      );
    });
  });

  const tokenBalances = Array.from(tokenBalanceMap.entries()).map(
    ([symbol, amount]) => ({
      symbol,
  ...
  txs.forEach((tx) => {
    ...
    if (tx.from?.toLowerCase() === address.toLowerCase())
      totalSent += isFinite(value) ? value : 0;
    if (tx.to?.toLowerCase() === address.toLowerCase())
      totalReceived += isFinite(value) ? value : 0;
  });

  const topTokenMap = new Map<
    string,
    { volume: number; transactions: number }
  >();
  txs.forEach((tx) => {
    tx.tokenTransfers?.forEach((transfer) => {
      const key = transfer.symbol || transfer.token;
      const existing = topTokenMap.get(key) || { volume: 0, transactions: 0 };
      const amountNum = Number(transfer.amount || 0);
      existing.volume += isFinite(amountNum) ? amountNum : 0;
      existing.transactions += 1;
      topTokenMap.set(key, existing);
    });
  });

  const topTokens = Array.from(topTokenMap.entries())
    .map(([symbol, data]) => ({
      symbol,
      address: symbol,
  ...
  return {
    wallet: {
      ...
      tokenBalances,
      lastTransaction: txs[0]?.timestamp || null,
      totalValueSent: totalSent.toFixed(6),
      totalValueReceived: totalReceived.toFixed(6),
    },
